...
x1=linspace(90,150,length(disp1));
for i=1:length(disp1)
    y1(i)=(2/3*Fmiu*Fnorm*((0.5*65)^3-(0.5*22.225)^3)/((0.5*65)^2-(0.5*22.225)^2)*2)*0.000001/0.25/cos(x1(i)/2/180*pi);
    %claculate the force of numeric model
end

for i=1:length(disp1)
    x1(i)=50*sin(x1(i)/2/180*pi)-25*sqrt(2); %change angle to displacement
 end
...
y1=-y1;
 ...
 ylabel('Force(KN)');
axis([0,14,-40,40]); %adjust it after first figure comes out
%------------------------------------------
title('T5-Displacement-Force-Figure');
...
hold on; grid on;
x1=linspace(90,150,length(disp1)); %reload angle
for i=1:length(disp1)
    y1(i)=(2/3*Fmiu*Fnorm*((0.5*65)^3-(0.5*22.225)^3)/((0.5*65)^2-(0.5*22.225)^2)*2)*0.000001/0.25/cos(x1(i)/2/180*pi);
